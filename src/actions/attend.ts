...
export default async (ctx: ContextMessageUpdate): Promise<void> => {
  try {
    ...
    const [playerCount = 0, personalWeaponsCount = 0] = input
      .split('.')
      .map((value) => +value || 0);
    ...
    const formatResultMessage = () => {
      ...
      const personalWeaponInfo =
        (personalWeaponsCount > 0 && ` 🔫 x${personalWeaponsCount}`) || '';

      return (
        `@${username} внесен в файл записи: 💂 x${playerCount}` +
        personalWeaponInfo
      );
    };
